...
bool Transaction::processTransfer(int senderAccountId, int receiverAccountId, double amount) {
    if (amount <= 0)
        return false;
    int first = std::min(senderAccountId, receiverAccountId);
    int second = std::max(senderAccountId, receiverAccountId);
    std::scoped_lock lock(accountLocks[first], accountLocks[second]);
